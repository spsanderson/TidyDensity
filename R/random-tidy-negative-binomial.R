...
#'
#' @details This function uses the underlying `stats::rnbinom()`, and its underlying
#' `p`, `d`, and `q` functions. For more information please see [stats::rnbinom()]
#'
#' @description This function will generate `n` random points from a negative binomial
...
#'
#' @name tidy_negative_binomial
NULL

#' @export
#' @rdname tidy_negative_binomial

tidy_negative_binomial <- function(.n = 50, .size = 1, .prob = .1, .num_sims = 1,
                                   .return_tibble = TRUE) {
  ...
  df[, y := stats::rnbinom(n = .N, size = size, prob = prob)]
  ...
  df[, p := stats::pnbinom(y, size = size, prob = prob)]

  # Compute the q-values for the p-values and add a column for q
  df[, q := stats::qnbinom(p, size = size, prob = prob)]
  ...
  attr(df, "tibble_type") <- "tidy_negative_binomial"
  attr(df, "ps") <- ps
  attr(df, "qs") <- qs

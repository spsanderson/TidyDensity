...
tidy_poisson <- function(.n = 50, .lambda = 1, .num_sims = 1, .return_tibble = TRUE) {
  ...
  df[, y := stats::rpois(n = .N, lambda = lambda)]
  ...
  df[, p := stats::ppois(y, lambda = lambda)]

  # Compute the q-values for the p-values and add a column for q
  df[, q := stats::qpois(p, lambda = lambda)]

...
#'
#' @seealso \url{https://www.itl.nist.gov/div898/handbook/eda/section3/eda3664.htm}
#'
#' @details This function uses the underlying `stats::rt()`, and its underlying
#' `p`, `d`, and `q` functions. For more information please see [stats::rt()]
#'
#' @description This function will generate `n` random points from a rt
...
#' @param .df Degrees of freedom, Inf is allowed.
...
tidy_t <- function(.n = 50, .df = 1, .ncp = 0, .num_sims = 1, .return_tibble = TRUE) {
  ...
  if (!is.numeric(deg_f) | !is.numeric(ncp) | deg_f <= 0) {
    rlang::abort(
  ...
  df[, y := stats::rt(n = .N, df = deg_f, ncp = ncp)]
  ...
  df[, p := stats::pt(y, df = deg_f, ncp = ncp)]

  # Compute the q-values for the p-values and add a column for q
  df[, q := stats::qt(p, df = deg_f, ncp = ncp)]

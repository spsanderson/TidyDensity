...
tidy_logistic <- function(.n = 50, .location = 0, .scale = 1, .num_sims = 1) {
  ...
  df <- dplyr::tibble(sim_number = as.factor(x)) %>%
    ...
    dplyr::mutate(y = list(stats::rlogis(n = n, location = location, scale = scale))) %>%
    dplyr::mutate(d = list(density(unlist(y), n = n)[c("x", "y")] %>%
      purrr::set_names("dx", "dy") %>%
      dplyr::as_tibble())) %>%
    dplyr::mutate(p = list(stats::plogis(unlist(y), location = location, scale = scale))) %>%
    dplyr::mutate(q = list(stats::qlogis(unlist(p), location = location, scale = scale))) %>%

...
#'
#' @name tidy_inverse_exponential
NULL

#' @export
#' @rdname tidy_inverse_exponential

tidy_inverse_exponential <- function(.n = 50, .rate = 1, .scale = 1 / .rate,
  ...
  df[, y := actuar::rinvexp(n = .N, rate = rate, scale = scale)]
  ...
  df[, p := actuar::pinvexp(y, rate = rate, scale = scale)]

  # Compute the q-values for the p-values and add a column for q
  df[, q := actuar::qinvexp(p, rate = rate, scale = scale)]

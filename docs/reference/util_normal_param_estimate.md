...
function and combined with the estimated normal data.
...
This function will attempt to estimate the normal gaussian mean and
standard deviation parameters given some vector of values.
...
#>   dist_type samp_size   min   max method               mu stan_dev shape_ratio
#>   <chr>         <int> <dbl> <dbl> <chr>             <dbl>    <dbl>       <dbl>
#> 1 Gaussian         50 -2.05  2.19 EnvStats_MME_MLE -0.130    0.973      -0.133
#> 2 Gaussian         50 -2.05  2.19 EnvStats_MVUE    -0.130    0.983      -0.132
```

...
This function estimates the parameters of a F distribution from the
...
- .x:
  ...
  A numeric vector containing the data to be fitted to an F
  distribution.

## Value

The AIC value calculated based on the fitted F distribution to the
provided data.
...
[`rf`](https://rdrr.io/r/stats/Fdist.html) for generating F-distributed

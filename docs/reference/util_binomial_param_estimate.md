...
This function will check to see if some given vector `.x` is either a
numeric vector or a factor vector with at least two levels then it will
cause an error and the function will abort. The function will return a
list output by default, and if the parameter `.auto_gen_empirical` is
set to `TRUE` then the empirical data given to the parameter `.x` will
be run through the
...
#> 1 Binomial         50     0     0     0        0 EnvSta…     0    50           0

...
tidy_autoplot(rw, .plot_type = "density")
```
...
```{r quantile-norm}
# Your data
data_vec <- c(5, 2, 8, 3, 9, 1, 7, 4, 6)

# Normalize to range [0, 1]
normalized <- tidy_scale_zero_one_vec(data_vec)
...
# Extract key statistics from the mixture data
mixture_stats <- mixture$data |>
  dplyr::filter(dist_type == "combined") |>
  dplyr::summarise(
